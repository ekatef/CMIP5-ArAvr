...
# contains the set of the functions	#
# to extract the model data 		#
#************************************
...
ListCMIPFiles <- function(CMIP5_Dir_Name = CMIP5_dir_name) {
	...
	if (length(filesInDir_names) == 0) {
		stop("The folder is empty. Check it please")
	}
...
TimeCorrections_in_days <- function(Date_to_Correct_days, 
	...
	# for consistency
	correction_days <- rep(0, times = max(length(current_year), 
		length(origin_year)))
...
ExtractModels <- function(CMIP5_Dir_Name = CMIP5_dir_name, 
	...
	filesInDir_names_with_path <- file.path(CMIP5_Dir_Name, filesInDir_names)

	StatBar <- txtProgressBar(min = 1, max = length(filesInDir_names), 
		style = 3)
...
# look to the CMIP6 dir for the simulation files
# return a dataframe with parameters of the models
ExtractModels2 <- function(CMIP5_Dir_Name = CMIP5_dir_name, 
	param_name) {
	...
	# for (i in 1:2) { # testing statement
		...
		file_attributes <- ncatt_get(file_nc_obj, varid=0)
		model_name <- file_attributes$source_id
		...
		SimParam_df[i,] <- list(FileName = filesInDir_names[i], 
...
FindCells <- function(ProcFile, DimCode = "lon", coord_min, coord_max) {
	if ((DimCode!="lon") & (DimCode!="lat")) {
		...
		i_min <- which.min(abs(coord_1d_array - coord_max))
		i_max <- which.min(abs(coord_1d_array - coord_min))
	}
	return(
		data.frame(min = min(i_min, i_max), max = max(i_min, i_max), 
		count = abs(i_max-i_min))
	)
}
...
ExtractGrid <- function(ProcFile, x_bnd, y_bnd) {
	...
	i_lon_range <- seq(from=i_lon_test_df$min,to = i_lon_test_df$max,
		length = i_lon_test_df$count)
	...
	i_lat_range <- seq(from = i_lat_test_df$min,to = i_lat_test_df$max,
		length = i_lat_test_df$count)
...
ReadModelFile <- function(CMIP5_Dir_Name, File_To_Proc_Name,
	...
	proc_grid <- ExtractGrid(ProcFile = file_nc_obj_procSsn,
		x_bnd = X_To_Proc_vct,
		y_bnd = Y_To_Proc_vct)	
	i_long_to_process <- FindCells(ProcFile = file_nc_obj_procSsn, 
		DimCode = "lon",
		coord_min = min(X_To_Proc_vct), coord_max = max(X_To_Proc_vct))
	i_lat_to_process <- FindCells(ProcFile = file_nc_obj_procSsn,
		DimCode = "lat",
		coord_min = min(Y_To_Proc_vct), coord_max = max(Y_To_Proc_vct))
	T_3D_array_to_process<-ncvar_get(file_nc_obj_procSsn, 
		varid = paramForProc_name, 
...
StichModelledFiles <- function(CMIP5_Dir_Name, Models_To_Calcul_List, 
	...
	model_data_list <- lapply(function(i) {
	...
	test_grid <- lapply(model_data_list, `[[`, "grid")
	test_time <- lapply(model_data_list, `[[`, "internal_time")
	test_T_3D <- lapply(model_data_list, `[[`, "T_3D")
	test_grid_lon <- lapply(test_grid, `[[`, "GridLon")
	test_grid_lat <- lapply(test_grid, `[[`, "GridLat")

	# check that all grids are the same
	# Reduce(identical, test_grid) returns FALSE if length(test_grid) > 2
	# that is why a special function is needed
	...
	test_time_calendar <- lapply(test_time, `[[`, "calendar_nc")

	# origins may be different for different time spans
	# so times in days and origin times are processed together
	...
	time_corrections <- lapply(
		{function(i) TimeCorrections_in_days(
					Date_to_Correct_days = test_time_days[[i]],
