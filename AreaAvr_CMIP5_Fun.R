# NB Path to Rscript.exe (e.g. "C:\Program Files\R\R-3.3.1\bin\i386\")
# should be added to the Environmental variable Path 
#
...
FindCells<-function(ProcFile,DimCode="lon",coord_min,coord_max) {
	...
	if ((DimCode!="lon") & (DimCode!="lat")) {
		...
		#for dim_min: find the corresponding grid cell
		i<-1
		repeat {
			i<-i+1
			if (i>N_dim) {
			...
			if ((coord_min<ProcFile$dim[[DimCode]]$vals[i-1])&(coord_min>=ProcFile$dim[[DimCode]]$vals[i])) {
			i_min<-i-1
			break
			}
			if ((coord_min>=ProcFile$dim[[DimCode]]$vals[i-1])&(coord_min<ProcFile$dim[[DimCode]]$vals[i])) {
		...
		#for dim_max: find the corresponding grid cell
		i<-1
		repeat {
			i<-i+1
			if (i>N_dim) {
			...
			if ((coord_max<=ProcFile$dim[[DimCode]]$vals[i-1])&(coord_max>ProcFile$dim[[DimCode]]$vals[i])) {
			i_max<-i
			break
			}
			if ((coord_max>ProcFile$dim[[DimCode]]$vals[i-1])&(coord_max<=ProcFile$dim[[DimCode]]$vals[i])) {
		...
		i_count<-i_max-i_min+1
		if (i_min!=i_max) {
			return(data.frame(min=i_min,max=i_max,count=i_count))	
		}
		else {
			message("Single coordinate value was found")
			return(data.frame(min=i_min,max=i_max,count=i_count))
		}
	}	
}
...
GetField_byTime<-function(ProcFile,x_min,x_max,y_min,y_max) {
	# the target variable is the last one (temperature/precip/runoff etc)
	param_name<-ProcFile$var[[length(ProcFile$var)]]$name
	i_long<-FindCells(ProcFile=ProcFile,DimCode="lon",coord_min=x_min,coord_max=x_max)
	i_lat<-FindCells(ProcFile=ProcFile,DimCode="lat",coord_min=y_min,coord_max=y_max)
	ParamField<-ncvar_get(ProcFile, varid=param_name, start=c(i_long$min,i_lat$min,1),
	 	count=c(i_long$count,i_lat$count,-1))
			#return(c(i_long,i_lat))
	return(ParamField)
}
#________________________________________________________________________________________________________________________
#
#		work with time
#________________________________________________________________________________________________________________________
# extract array of time moments (actually, that's a supplementary function, is used in ExtractSeason)
...
ExtractTime<-function(ProcFile) {
	...
	# ??? could gsub be more helpful?
	# cut the tail of the string "days since..."
	string_tail<-strsplit(time_units,split="since ")[[1]][2]
	# take the date from the string
	N_time_begin<-strsplit(string_tail,split=" ")[[1]][1]
	...
	#N=year_N-year_0+1
	date_result<-list(firstYear=year_0,lastYear=year_N,numberOfMoments=J,year_byDates=year_asDate,months=month_i,
		tau_daysAsDate=tau)
	return(date_result)
}
#________________________________________________________________________________________________________________________
# Seasons should be set via months of the current year
# Work with winter (Dec-Feb) requires some modifications
# @ProcFile is a netCFD file to be processed (should be opened first)
...
ApproxForSeason<-function(ProcFile,x_min,x_max,
	y_min,y_max,SeasonPeriods=c("Èþíü","Èþëü","Àâãóñò"),
	FirstSeasonDay_date="-06-01",LastSeasonDay_date="-08-31",i_OfYear) {
	# TODO Check of correspondance between the Seasonal period 
	# and the Seasonal dates
	# read filed parameter data
	T_3D<-GetField_byTime(ProcFile,x_min=x_min,x_max=x_max,
		y_min=y_min,y_max=y_max) # 3D array
	# read time data
	...
	# TODO Do not mix local & global variables!!!
	ApproxResult<-function(i,j) {
	...
	test_x<-list()
	for (j in (1:lengthY)) {
		test_x[[j]]<-unlist(lapply(FUN=function(i) ApproxResult(i,j=j),X=1:lengthX))
	}
	test_mx<-matrix(unlist(test_x[1:lengthX]),byrow=TRUE,nrow=lengthY)
	return(test_mx)
}
#